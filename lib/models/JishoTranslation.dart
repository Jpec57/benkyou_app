...
class JishoTranslation {
  ...
  static Future<List<JishoTranslation>> getJishoTransLationListFromRequest(
      String word) async {
    if (word.isEmpty) {
      return [];
    }
    var res = await get('https://jisho.org/api/v1/search/words?keyword=$word');
    ...
    for (var matchingItem in matchingData) {
      ...
      for (var sense in senses) {
        ...
        jishoList.add(JishoTranslation(
            jap['word'], jap['reading'], englishDefStringList));
      }
    }
    return jishoList;
  }
